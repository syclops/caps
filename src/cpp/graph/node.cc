...
{
  return follow_out_edge(std::move(label)) == target;
}

bool Node::has_out_label(std::string label) const
{
  return contains(out_edges_, label);
}
...
const std::set<std::pair<std::string, std::weak_ptr<Node>>, in_edge_comparator>&
Node::get_in_edges() const
{
  return in_edges_;
}

const std::map<std::string, std::shared_ptr<Node>>& Node::get_out_edges() const
{
  return out_edges_;
}

ReverseIterable<std::set<std::pair<std::string, std::weak_ptr<Node>>,
                         in_edge_comparator>>
...
ReverseIterable<std::map<std::string, std::shared_ptr<Node>>>
