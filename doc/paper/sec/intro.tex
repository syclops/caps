...
\emph{\ac{mitm} attack}, the very problem that \iac{pki} aims to solve.
...
simultaneous failure of $n-1$ \acp{ca} cannot enable \iac{mitm} attack.
...
Even with the above multi-certificate-chain system, the number of certificate
...
%\item We conduct a study of the existing Web \ac{pki} to assess the current
  ...
  %deployment.
\item We propose a design for \ac{caps}, a system that enables the smooth
