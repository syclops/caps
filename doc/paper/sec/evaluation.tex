...
\begin{figure*}[thb]
  ...
  \caption{Time Elapsed during Connection Establishment \jay{TODO:
      Figure out how to make these readable, without taking up too
      much more space. Maybe move to Appendix? In that case, will need
      to restructure the text in the paper.}}
...
\item the number of certificate chains the server \bryan{which server?} \jay{isn't it clear from the fact that we have written a TLS extension, that we are talking about the server that the user is trying to establish a connection to?} sends (\policy) during the ServerHello message,
...
\bryan{A bit unclear, given that most params don't apply. Need to more precisely define the baseline.} \jay{Not sure I understand}
the \ac{name} \ac{tls} extension. We
tested this both over the internet (by connecting to a virtual private server \jay{with latency varying between 30 to 300 ms, based upon network conditions}, referred to as \emph{hophop}), as well as over the local loopback
interface (referred to as \emph{localhost}) \bryan{why is the local loopback one interesting?}.
\jay{[TODO: Restructure this] The tests over the internet to \emph{hophop} provide an indication of the effect of the extension on ``real world'' servers, which are usually accessed over WAN, whereas the \emph{localhost} tests provide a lower bound on time added due to sending/receiving/processing extra data.}
A total of 15385 TLS connections were established for our testing \jay{5768 over hophop, 9617 over localhost}.

Our results, shown in Figure~\ref{fig:evaltlsext}, show that in
comparison to the mean time elapsed, there is an approximately 5\%
increase in connection establishment time \jay{For hophop, we get an
  additional 11ms on average; for localhost, we get an additional
  1.2ms}.
%
