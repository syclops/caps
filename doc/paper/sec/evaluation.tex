...
latency overhead, mainly due to the extra data sent over the
network. However, since our \ac{tls} extension does not add any extra
round-trips to the handshake, the time added is small. \jay{[The null
  hypothesis on the Student's t-test says that the two distributions
  are identical. With such a low p-value, isn't the null-hypothesis
  invalidated? In particular, doesn't this mean that the distributions
  are, in fact, \emph{not} identical?] [OLD: Additionally, by
  performing the two-tailed paired Student's t-test, we see that
  statistically, the latency with and without the \ac{name} \ac{tls}
  extension appears to be drawn from the same distribution with the
  same mean. We obtain a p-value of $3.49 \times 10^{-130}$ and
  $1.37 \times 10^{-42}$ respectively for \emph{localhost} and
  \emph{hophop} for this test.]}
