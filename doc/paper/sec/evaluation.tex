...
To measure the \bryan{Need to motivate/remind}
...
Additionally, we constructed sample sets of \jay{domain names} based
on four parameters: \bryan{Make the \texttt{inparaenum} numbering bold}
\begin{inparaenum}
\item the number of proofs requested by the client (\numlas),
\item the number of certificate chains the server \bryan{which server?} sends (\policy),
...
on the size and number certificate chains, as well as the number of
