...
regular network interface \jay{(WiFi ; opens up a tunnel from my
  server to me, and then runs the tests)}, as well as over the local
loopback interface.
...
\jay{TODO: Add in a table/graphs for results; explain table/graphs
  here. If we directly add in a table with all possible dimensions, it
  would be too large. Might be better to make into 4 different
  graphs.}

\jay{Restructure this?} Overall, via the two-tailed paired Student's
t-test, we see that statistically, the latency with and without the
\ac{name} \ac{tls} extension is drawn from the same distribution, with
the same mean. We obtain a p-value of $3.49 \times 10^{-130}$ and
$1.37 \times 10^{-42}$ respectively for \jay{localhost} and
\jay{hophop} for this test.
