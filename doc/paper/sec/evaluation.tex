...
with latency varying from 30 to 300 ms \jay{remove this: ``, referred to as \emph{WAN}''?}), as well
as over the local loopback interface \jay{remove this: ``(referred to as \emph{localhost})''?}. The
tests over the internet (\emph{WAN}) provide an indication of the effect of
the extension on ``real world'' servers, \jay{remove this: ``which are usually accessed over WAN,''}
