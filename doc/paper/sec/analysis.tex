...
by simple countermeasures, or out of the scope of \ac{name}.
...
allow an adversary to compromise many \acp{ca} at once. Beyond domains being
able to prepare for this possibility with an appropriately high value of
$\policy$, such a widespread compromise would be quickly detected by public
...
previously logged certificates. \ac{ct} proposes several auditor roles that
monitor logs for any such misbehavior, with anyone being able to take on this
role~\cite{rfc6962}. Recording fraudulent certificates may cause a log
...
its own root certificates, or display arbitrary pages. Even with \ac{name}, an
