...
\ac{pki}. To provide a smooth transition from the current \ac{pki}, we created
\emph{\ac{name} policies}, which are based on a simple and intuitive approach:
\emph{treat whichever public key is backed by the most independent certificate
chains\footnote{This term means that the certificate chains share no public keys
except at the leaf.} in the current \ac{pki} as authoritative in the improved
\ac{pki}}. A domain wanting to increase client confidence in one of its public
keys can then simply obtain more independent certificates for that key, and the
log aggregators will update their \ac{name} policies for that domain.
...
where \hostname is a domain name and \policy is the \emph{\ac{name} policy
  value}, the number of independent certificate chains backing an authoritative
  key for \hostname.\footnote{\hostname may have more than one authoritative
  key; if certificates $C_1$ and $C_2$ are both backed by three independent
chains, for example, then both of their public keys are treated as authoritative
...
\item a set of \acp{fqdn} (hereafter names) known to have deployed \ac{https}
  (by virtue of having a known public-key certificate for the Web \ac{pki}), and
...
\httpsset and \multicertset are unordered sets of valid\footnote{A valid
  name is a Unicode or ASCII string up to 253 bytes in length overall, and
...
The log aggregator obtains the names from the current and valid
certificates in the databaseand converts internationalized domain names to ASCII
using the IDNA conversion process~\cite{rfc5891}; the resulting set of distinct
names is \httpsset. The log aggregator also analyzes the certificate chains
...
improved \ac{pki} and may contain all names in DNS, the log aggregator must
succinctly represent this set to minimize the bandwidth and storage burdens on
...
flagging a site as an attack when there is none) and for the scale of data we
considered, resulted in too large of a storage burden
...
parameters would minimize the required storage space, but would also require
...
In our log aggregator prototype, we ultimately chose to represent the signaling set as a
data structure known as a \emph{\acf{dafsa}}, which succinctly stores a set of
...
large set of names, much of the redundancy in \httpsset can be removed with
...
\states contain a symbol \multicertsymbol. Then, if there exists sequence of
states and symbols $(s_1, \ldots, s_n), (\sigma_1, \ldots, \sigma_{n-1})$ that
satisfies
\begin{inparaenum}
\item $\transfunc(s_i, \sigma_i) = s_{i+1}$ for each $i$ where $1 \le i < n$,
\item $s_1 = \initstate$,
\item $\sigma_1 \| \dots \| \sigma_{n-1} = \hostname$, and
\item $\transfunc(s_n, \multicertsymbol) \in \finalstates$,
\end{inparaenum}
we say that $\query(\hostname) = \multicert$. If no such sequence exists, but
one satisfying the first three conditions and $s_n \in \finalstates$ exists,
...
work~\cite{daciuk2000incremental}. Though this work assumes transitions based on
a single character, we consider the possibility of multi-character symbols and
thus our symbol set $\symbolstrings = \bigcup_{i=0}^{253} \symbols^i$, where
\symbols is the set of all ASCII characters allowed in domain
...
\paragraph{Transition Compaction}

We extend the design of prior work using \emph{transition compaction}, which
aims to minimize the \ac{dafsa} representation by reducing the representation
size of each transition. A transition in the representation consists of a symbol
and destination state. Because we found that both the symbols and destination
states had a long-tailed distribution as described below and in
\autoref{sec:evaluation:implementation}, we chose to represent both of these
items using variable-length encodings. We represent each unique symbol in the
\ac{dafsa} with a Huffman code.

We found that by creating an ordering over all of the states in the \ac{dafsa}
and considering the difference between the source and destination states with
respect to their positions in the ordering, we could achieve a more compact
representation of the destination state on average. Specifically, we used a
Huffman code to encode the destination state as a number of states forward or
backward in the ordering. To do this, we started at the initial state \initstate
and performed a depth-first search, choosing each branch based on the longest
path that could be formed by following the given transition. This ensured that
the maximum number of transitions went from a given state to its following state
in the ordering, reducing the entropy of the distribution and thus minimizing
the representation size of the destination state.
...
We also extend the design of prior work with \emph{path compaction}, which
...
= s_{i+1}$ or $\transfunc(s_{i+1}, \sigma_i) = s_i$. A walk where
...
states, which we writes as \symboldh and \destdh, respectively. If we know the
...
$1 < i < m$, $s_i$ only had one incoming and one outgoing transition. Because
building the \ac{dafsa} as described in prior work resulted in a significant
number of isolated paths, performing path compaction with all such paths
resulted in a size savings of \steve{XX\%} of the original \ac{dafsa} size.
...
outgoing transitions (or vice versa) yielded more modest but still significant
...
aggregators again rely on the data from public sources for this information,
...
To establish a connection to a domain, a client first looks up the domain's name
as a query to the signaling set. Due to the way that querying occurs in the
...
hence the domain has a policy value greater than 1). If this query returns
\onecert or \multicert, then the client performs the \ac{name} handshake to
establish a connection with the domain.

The \ac{name} handshake protocol allows a client to verify a domain's
...
\numlas, indicating a number of policy proofs that the domain should send back.
...
The domain then sends back a ServerHello message, which contains an extension
message as a response to \numlas. The extension message contains recent policy
proofs from \numlas distinct log aggregators, as well as a number of certificate
chains, which allows the client to verify the domain's policy value and the
certificate chains that back this value. Specifically, a domain with hostname
...
chain will be sent in the ServerCertificate message of the handshake.
...
\item the name for each policy proof equals the domain's name,
...
\end{inparaenum}
If the above checks pass, the client can then continue with the handshake, which
requires the client to verify an additional certificate chain in the
ServerCertificate message and perform all other checks required by the \ac{tls}
handshake protocol.
...
policies, which can be quite large in these systems. Moreover, in \ac{name},
this bootstrapping can take place at any time during deployment, which means
that in the case of a lost or compromised private key, a domain can simply
obtain \policy + 1 new, independent certificate chains. This contrasts with
previous proposals, which often rely on heavyweight processes that involve
