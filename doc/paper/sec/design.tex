...
policies take a simple and intuitive approach: \emph{treat whichever public key
  is backed by the most independent certificate chains\endnote{This term means
...
increase client confidence in one of its public keys can then simply obtain more
independent certificates for that key, and the log aggregators will update their
\ac{name} policies for that domain.
...
where \hostname is a domain name and \policy is the \emph{\ac{name} policy
  ...
  key for \hostname.\endnote{\hostname may have more than one authoritative
...
\httpsset and \multicertset are unordered sets of valid\endnote{A valid
...
maintaining a list of certificates valid on a given day.\endnote{A certificate
...
set of strings and supports efficient membership queries in this set, and can be
...
large set of names, much of the redundancy in \httpsset can be removed with this
...
make two additional design changes to our \ac{dafsa} representation that further
reduce its size.
...
names.\endnote{Recall that DNS names can be a maximum of 253 characters.}
...
timestamping and signing them.\endnote{For efficiency, the log aggregator can
