...
within the existing \ac{tls} handshake.\footnote{Exported authenticators as
  mentioned in \autoref{sec:background:issuance} could be used in place of a
  custom extension. However, this requires the client to establish \iac{https}
  connection with a potentially malicious endpoint.} During the initial
