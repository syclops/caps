...
ecosystem, most notably \emph{certificate logs}, who record nearly\endnote{A
  log records all certificates submitted with chains rooted in \iac{ca} trusted
...
\ac{tls} handshake protocol.\endnote{Two extensions may be used: a
...
\ac{https}.\endnote{\url{https://www.eff.org/https-everywhere/}} The extension
...
preloading. \steve{Add a reference to a graphic in \autoref{sec:evaluation} that
justifies the claim in the previous sentence.}
...
attempt.\endnote{\url{https://mybrowseraddon.com/smart-https.html}} Smart
...
Public-key pinning is \iac{tofu} approach that, similarly to \ac{hsts}, allows a
...
\ac{hpkp} was slated for removal from Google Chrome, due in part to these
pitfalls~\cite{palmer2017intent}. (\ac{tack} was never widely adopted.)

Log-based authentication extends functionality of \ac{ct} to enable logs to
...
Furthermore, these systems propose limited deployment strategies, expecting all
...
%guidelines,\endnote{\url{https://hstspreload.org/}} this process is prone to
...
%exist.\endnote{\url{http://secspider.verisignlabs.com/stats.html}, retrieved 1
